...
var Errors = function () {
  ...
  function Errors(errCode, supString) {
    var functionName = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : getFunctionName(NUMBER_OF_LEVEL_TO_GO_BACK_ERROR_CLASSIC);
  ...
  (0, _createClass3.default)(Errors, [{
    ...
    value: function getColoredErrorString() {
      ...
      if (isFirst || this.errorCode !== 'ESTACKTRACE') {
        strsParts.push(Utils.monoline(['--> Error['.red, ('' + this.errorCode).yellow, ']: ['.red, ('' + this.getMeaning()).yellow, ']\n'.red]));
      }
      ...
      if (isFirst) {
        ...
        finalArrayToDisplay.push(Utils.monoline(['TRACE: '.bold.underline.red, '--------------------------------------------------------------'.bold.red, '\n']));
        ...
        finalArrayToDisplay.push(Utils.monoline(['---------------------------------------------------------------------'.bold.red, '\n']));
    ...
    value: function deserialize(str) {
      var obj = Utils.convertStringToJSON(str);
    ...
    value: function handleStackTraceAdd(err, errToAdd) {
      var funcName = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : getFunctionName(NUMBER_OF_LEVEL_TO_GO_BACK_ERROR_HANDLE_STACK_TRACE);
